...
\end{vmatrix}
= \frac{1}{\sqrt{2\pi}}\exp\left(-\frac{r^2}{2}\right)|r| = \left(r\exp\left(-\frac{r^2}{2}\right)\right)\left(\frac{1}{\sqrt{2\pi}}\right)
$$
Since the portions with $r$ and $\theta$ are separable, $R$ and $\theta$ are independent with PDFs 
$$f_R(r) = r\exp\left(-\frac{r^2}{2}\right) \quad f_\theta(\theta) = \frac{1}{\sqrt{2\pi}}$$
...
&= (x/c)^{n-1}
\end{align*}
so the PDF is $f_{U_{(2)}|U_{(3)}} = (n-1)(x/c)^{n-2}\cdot(1/c)$. Interestingly, this is the distribution of $cX$ where $X \sim \text{Beta}(n-1, 1)$, which you can verify with a change of variables.
}
